...
namespace san
{
		...
		: Gtk::Window(cobject), builder(builder)
	{
		...
		btn_add_connection->signal_clicked().connect
			(sigc::mem_fun
	...
	void NewConnectionWindow::on_btn_add_connection_clicked()
	{
		set_adding_mode();
	}
	...
	{
		Glib::RefPtr<Gtk::TreeSelection> selection = tree_connections.get_selection();
	...
	void NewConnectionWindow::set_editing_mode()
	{
		mode = Mode::Editing;
	}
	...
	bool NewConnectionWindow::can_add_new_connection() const
	{
		...
		if (san::Connections::instance()
			->exists(text_connection_name->get_text()))
			return false;
	...
	{
		text_connection_name->set_text("Connection "
