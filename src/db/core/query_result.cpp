...
QueryResult::get_columns_data(pqxx::connection& conn,
    ...
    for (const auto& row : result) {
        ...
        columns.emplace(
            column_name,
            sancho::db::ColumnMetadata(character_maximum_length, is_nullable));
    }
...
void QueryResult::handle_results(const pqxx::result& result) {
    ...
    for (unsigned i = 0; i < result.columns(); i++) {
        ...
        if (use_columns && IN_MAP(columns_data, result.column_name(i))) {
            char_length = columns_data.at(result.column_name(i))
                              .get_character_maximum_length();
            is_nullable =
                columns_data.at(result.column_name(i)).get_is_nullable();
        }
