...
class Connections {
  public:
    ...
    void add(const Glib::ustring& name, const std::string& host,
             ...
             const std::string& sslmode, bool save_password);

    static Connections* instance() { return &ins; }

    const std::map<Glib::ustring,
                   std::shared_ptr<sancho::db::ConnectionDetails>>&
    ...
    bool
    can_update_conn_details(const Glib::ustring& old_conn_name,
    ...
    void update_conn(const Glib::ustring& old_conn_name,
                     ...
                     const std::string& port, const std::string& sslmode,
                     bool save_password);
